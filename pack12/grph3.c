//USEGAE: ./grph3.c (START_RESNUM) (END_RESNUM) (COEFFICIENT)
...
int result2(char *str1,char *str2,char *str3){
	FILE *fa;
        char rename[100];
		if(str3==NULL){
		str3="?";
		}
        sprintf(rename,"./result12/rslt/%s_%s_%sg3.dat",str3,str1,str2);
...
// make a dat file to make a graph
	int d[3],e[3],f[3],g[3];
		for(j=1;j<4;j++){
			d[j-1]=count(xb,yb,zb,ca,sa,i,j);
			e[j-1]=count(xa,ya,za,cb,sb,h,j);
			f[j-1]=count2(xb,yb,zb,ca,0,s1,i,j);
			g[j-1]=count2(xa,ya,za,cb,0,s2,h,j);
		} 
